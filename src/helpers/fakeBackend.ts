...
const fakeBackend = () => {
  ...
  mock.onPost(url.POST_FAKE_REGISTER_OTP).reply(config => {
    const user = JSON.parse(config["data"]);
    users.push(user);
    return new Promise((resolve, reject) => {
      setTimeout(() => {
  ...
  */
  ...
  // mock.onGet(url.ADD_BOOKMARK).reply(config => {
  //   debugger
  //   return new Promise((resolve, reject) => {
  //     if (bookmarks) {
  //       resolve([200, bookmarks]);
  ...
  */
  ...
  mock.onPost(new RegExp(`${url.ADD_BOOKMARK}/*`)).reply(config => {
    const data = JSON.parse(config["data"]);

    return new Promise((resolve, reject) => {
      if (data) {
        let modifiedC = [...conversations];
        let contact = contacts.find(e => e.id == data.meta["receiver"])
        let modifiedB = [...bookmarks]
        modifiedB.push({
        ...
        // }
        // onChangeConversations(modifiedC);
        resolve([200, "Message bookmarked"]);
      } else {
        reject(["Your id is not found"]);
      }
    });
