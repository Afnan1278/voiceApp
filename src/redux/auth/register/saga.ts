...
import { postFakeRegister, postJwtRegister, postFakeOtp } from "../../../api/index";
...
function* registerOTP({ payload: { user } }: any) {
  try {
    if (process.env.REACT_APP_DEFAULTAUTH === "firebase") {
      const response: Promise<any> = yield call(
        fireBaseBackend.registerUser,
      ...
      const response: Promise<any> = yield call(postFakeOtp, user);
      yield put(
        authRegisterApiResponseSuccess(
          AuthRegisterActionTypes.REGISTER_OTP,
          response
        )
      );
    }
...
export function* watchUserRegisterOTP() {
  yield takeEvery(AuthRegisterActionTypes.REGISTER_OTP, registerOTP);
}

function* registerSaga() {
  yield all([fork(watchUserRegister), fork(watchUserRegisterOTP)]);
}
