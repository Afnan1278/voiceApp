...
function* addBookmark({ payload: data }: any) {
  try {
    const response: Promise<any> = yield call(addBookmarkApi, data);
    yield put(bookmarksApiResponseSuccess(BookmarksActionTypes.ADD_BOOKMARK, response));
    yield call(showSuccessNotification, response + "");
  } catch (error: any) {
    yield call(showErrorNotification, error);
    yield put(bookmarksApiResponseError(BookmarksActionTypes.ADD_BOOKMARK, error));
  }
}
...
export function* watchAddBookmark() {
  yield takeEvery(BookmarksActionTypes.ADD_BOOKMARK, addBookmark);
}
